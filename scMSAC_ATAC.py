...
class scMSAC(nn.Module):
    def __init__(self, input_dim1, input_dim2, encodeLayer=[], decodeLayer1=[], decodeLayer2=[], encodeHead=3,
                 encodeConcat=False,
                 activation='elu', z_dim=32, alpha=1.,beta=0.05, gamma=0.1, sigma1=2.5, sigma2=1.5, tho=1, device="cuda"):
    ...
    def aeForward(self, x,x2, g):
        ...
        x_d = x2 + torch.randn_like(x2) * self.sigma2
    ...
    def train_model(self, X,X2, G,dataloader, lr=0.001, train_iter=400,
        ...
        for i in range(train_iter):
            ...
            for it, sg in enumerate(dataloader):
                x1 = sg.ndata["x1"].cuda()
                x2 = sg.ndata["x2"].cuda()
                X_raw1 = sg.ndata['X_raw1'].cuda()
                X_raw2 = sg.ndata['X_raw2'].cuda()
                size_factor1 = sg.ndata['size_factor1'].cuda()
                size_factor2 = sg.ndata['size_factor2'].cuda()
                sg = sg.to('cuda')
                ...
                if verbose:
                    ...
                    loss_zinb1 = self.zinb_loss(x=X_raw1, mean=mean1, disp=disp1, pi=pi1, scale_factor=size_factor1)
                    loss_zinb2 = self.zinb_loss(x=X_raw2, mean=mean2, disp=disp2, pi=pi2, scale_factor=size_factor2)
                    loss_zinb_val = loss_zinb1.data + loss_zinb2.data *0.1
    ...
    def fit(self, X,X2, G,dataloader, n_clusters,
        ...
        for epoch in range(num_epochs):
            ...
            for it, sg in enumerate(dataloader):
                ...
                # dc = self.dc(z)
                # gcn_loss = F.binary_cross_entropy_with_logits(dc, sgg)
                loss = loss_zinb1 + loss_zinb2 * self.beta + self.gamma * loss_cluster
                optim_adam.zero_grad()
                loss.backward()
                torch.nn.utils.clip_grad_norm_(self.mu, 1)
                optim_adam.step()
                total_loss += (loss_zinb1 + loss_zinb2*self.beta ).data / num + loss_cluster.data / num
                kl_val += loss_cluster
