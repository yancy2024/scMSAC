...
from scMSAC_ATAC import scMSAC
import h5py
import torch
from utils import *
...
if __name__ == "__main__":
    ...
    parser.add_argument('--train_iter', default=200, type=int, help='iteration of pre-clustering stage')
    ...
    parser.add_argument('--decodeLayer1', nargs="+", default=[64,128,512], type=int, help='decoder layer size')  # [64,128,512]
    parser.add_argument('--decodeLayer2', nargs="+", default=[64,128,512], type=int, help='decoder layer size')
    ...
    parser.add_argument('--z_dim', default=16, type=int, help='dimension of latent space')
    parser.add_argument('--verbose', default=True, type=bool)
    parser.add_argument('--save_dir', default='./single')
    ...
    parser.add_argument('--weights_name', default='PBMC_x.pth.tar')
    parser.add_argument('--gamma', default=0.001, type=float, help='coefficient of clustering loss')
    parser.add_argument('--clustering_iters', default=100, type=int, help='iteration of clustering stage')
    parser.add_argument('--sigma1', default=1, type=float, help='noise added on data for denoising autoencoder')
    ...
    ###read data
    # data_mat = h5py.File(
    #     "/root/autodl-tmp/Multi-omics/RNA_ATAC/SMAGESeq_10X_pbmc_granulocyte_plus.h5")  # /home/ubuntu/Q8/Multi-omics/CITESeq_pbmc_spector_all.h5
    ...
    x1 = pd.read_csv("/root/autodl-tmp/Multi-omics/RNA_ATAC/pbmc10k/RNA.csv", index_col=0).T.values
    x2 = pd.read_csv("/root/autodl-tmp/Multi-omics/RNA_ATAC/pbmc10k/activity.csv", index_col=0).values.T
    y = pd.read_csv("/root/autodl-tmp/Multi-omics/RNA_ATAC/pbmc10k/labels.csv", index_col=0)
    # # if label is 'string':ATAC
    ...
    adj = pd.read_csv('/root/autodl-tmp/Multi-omics/RNA_ATAC/pbmc10k/wnn_adj.csv', index_col=0).values    #10X10k
    ...
    model = scMSAC(input_dim1=adata.n_vars, input_dim2=adata1.n_vars, encodeLayer=args.encodeLayer,
                   ...
                   encodeConcat=args.concat, gamma=args.gamma, beta=1, activation="elu", z_dim=args.z_dim, sigma1=args.sigma1,
    ...
    ###clustering stage
    y_pred, final_latent, final_loss, epoch, z3, z4 = model.fit(x1, x2, G=G,dataloader=dataloader,
    ...
    print('Final Clustering: ACC= %.4f, NMI= %.4f, ARI= %.4f, Loss = %.8f, Epoch = %.i' % (acc, nmi, ari, final_loss, epoch))
    ...
    # final_latent = pd.DataFrame(final_latent)
    # final_latent.to_csv(args.save_dir + "/" + data_name + "_atac.csv", index=True, header=True)


    ###output predicted labels and embedding
    if args.final_latent_file != -1:
        # final_latent = model.encodeBatch(torch.tensor(count, dtype=torch.float32), adj).data.cpu().numpy()
        final_latent = final_latent.data.cpu().numpy()
        final_latent = pd.DataFrame(final_latent)
